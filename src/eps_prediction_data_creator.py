...
def create_eps_based_prediction_data(prediction_horizon_in_days):
    tickers = _get_list_of_tickers_from_indexed_EPS()
...
def _append_rows_to_csv(prediction_data_file, ticker, prediction_horizon_in_days):
    ...
    with open(indexed_eps_file_path, 'r') as indexed_eps_file:
        for line in indexed_eps_file:
            ...
            base_price = _find_price_before(close_prices_dict, eps_date)
            price_at_horizon = _find_price_at_horizon(close_prices_dict, eps_date, prediction_horizon_in_days)
...
def _find_price_before(prices_dict, eps_date):
...
if __name__ == '__main__':
    create_eps_based_prediction_data(5)
